...
def process_frame(frame, model, width, height, progress):
...
def processVideo():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        transformed_frames = []
        # for frame in frames:
        #     task = executor.submit(process_frame, frame, model, width, height, progress)
        #     transformed_frames.append(task)

        # # Get the results as they become available
        # for task in concurrent.futures.as_completed(transformed_frames):
        #     transformed_array = task.result()
        #     video_writer.write(transformed_array)

        # create array of task objects along with indices
        for i in range(len(frames)):
            task = executor.submit(process_frame, frame, model, width, height, progress)
            transformed_frames.append([i, task])
        res = []
        # as tasks are finished, append to array
        for index, task in concurrent.futures.as_completed(transformed_frames):
            res.append([index, task.result()])
        #sort the array by index so that the frames are in the correct order
        
        res.sort()
        # write the sorted frames to the video
        for index, t in res:
